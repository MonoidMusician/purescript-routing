...
import Routing.Generic (Parameter(..), routingGeneric)
...
derive instance genericFooBar :: Generic FooBar _

instance showFooBar :: Show FooBar where
  show (Foo num q) = "(Foo " <> show num <> " " <> show q <> ")"
  show (Bar bool (Parameter str) (Parameter s)) = "(Bar " <> show bool <> " " <> show str <> " " <> show s <> ")"
...
{-
bidi2 :: forall m. Combinators m => MatchClass m => m FooBar
bidi2 = let discard = C.discard in do
...
main = do
  ...
  testroundtrip (Baz (1.2:12.0:3.0:Nil)) $ "baz/1.2/12.0/3.0":|"baz/1.2/12.0/3.0/":Nil
